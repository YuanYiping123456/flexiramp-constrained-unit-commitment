...
function re_flexramp_scucmodel(NT::Int64, NB::Int64, NG::Int64, ND::Int64, NC::Int64, units::unit, loads::load, winds::wind, lines::transmissionline, config_param::config)
	...
	plentycoffi_2 = config_param.is_WindsCuttingCoefficient * 1e10
	ρ⁺ = c₀ * 2
	ρ⁻ = c₀ * 2
	...
	@constraint(scuc,
		...
		sum(sr⁺[(1 + (s - 1) * NG):(s * NG), t])>=0.05 * units.p_max[i, 1] * x[i, t])
	@constraint(scuc,
		[s = 1:NS, t = 1:NT],
		sum(sr⁻[(1 + (s - 1) * NG):(s * NG), t])>=
		0.05 * (config_param.is_Alpha * forcast_reserve[s, t] + config_param.is_Belta * sum(loads.load_curve[:, t])))
	...
	p1 = Plots.plot(xdata, loads.load_curve[1, :], label = "load")
	...
	filepath = "D:\\IEEE_Transactions Research\\2 ieee pes\\code\\highpenetration_DER"
	p5 = Plots.plot(p1, p2, p3, p4, layout = (2, 2))
	# Plots.savefig(p5, filepath * "\\fig\\flexramp_requirement.png")
	Plots.savefig(p5, "D:\\IEEE_Transactions Research\\2 ieee pes\\master - 1\\code\\highpenetration_DER\\fig\\re_flexramp_requirement.png")
	...
	# @constraint(scuc,
	# 	[s = 1:NS, t = 1:(NT - 1)],
	# 	sum(sr⁺[(1 + (s - 1) * NG):(s * NG), t + 1]).>=sum(units.p_max[g, 1] .* u[g, t] for g in 1:NG))

	# @constraint(scuc,
	# 	[s = 1:NS, t = 1:(NT - 1)],
	# 	sum(sr⁻[(1 + (s - 1) * NG):(s * NG), t + 1]).>=sum(units.p_min[g, 1] .* v[g, t] for g in 1:NG))
	...
	@constraint(scuc,
		[s = 1:NS, t = 1:NT],
		sum(sr⁻[(1 + (s - 1) * NG):(s * NG), t]).>=sum(units.p_min[g, 1] .* v[g, t] for g in 1:NG))
	...
	open("D:\\IEEE_Transactions Research\\2 ieee pes\\master - 1\\code\\highpenetration_DER\\res\\re_flexramp_calculation_result.txt", "w") do io
		writedlm(io, [" "])
